...
const DEFAULT_LANYARD_URL = '/assets/lanyard.png';
...
function Band({
  ...
  const sourceTexture = useTexture(lanyardImage || DEFAULT_LANYARD_URL);
  ...
  const cardMap = useMemo(() => {
    ...
    if (frontImage && frontTex.image) drawFitted(frontTex.image as DrawableImage, FRONT_UV_RECT);
    if (backImage && backTex.image) drawFitted(backTex.image as DrawableImage, BACK_UV_RECT);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      curve.current.points[1].copy(getLerped(j2.current));
      curve.current.points[2].copy(getLerped(j1.current));
